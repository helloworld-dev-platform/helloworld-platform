...
public class LevelTestProblemResponse {
  ...
  private List<LevelTestChoiceResponse> choices;


  /**
   * 책임 분리: Problem 엔티티를 DTO로 변환하는 정적 팩토리 메소드
   */
  public static LevelTestProblemResponse from(Problem problem, List<Choice> choiceEntities) {
    // Choice Entity 리스트 -> DTO 리스트 변환
    List<LevelTestChoiceResponse> choiceDtos = (choiceEntities == null || choiceEntities.isEmpty())
        ? Collections.emptyList()
        : choiceEntities.stream().map(LevelTestChoiceResponse::from).collect(Collectors.toList());

    return LevelTestProblemResponse.builder()
  ...
  static class LevelTestChoiceResponse {
    ...
    public static LevelTestChoiceResponse from(Choice choice) {
      return LevelTestChoiceResponse.builder()
