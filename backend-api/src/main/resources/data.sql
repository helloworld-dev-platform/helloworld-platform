...
-- #####################################################################
-- 2. STEPUP_LARGE_CATEGORY (스텝업 대분류) - ID 1~4
-- #####################################################################
...
-- #####################################################################
-- 3. STEPUP_MEDIUM_CATEGORY (스텝업 중분류) - ID 1~8
-- #####################################################################
...
-- #####################################################################
-- 4. STEPUP_SMALL_CATEGORY (스텝업 소분류) - ID 1~16
-- #####################################################################
-- 중분류 1 (ID 1, Python 변수)
...
-- #####################################################################
-- 5. PROBLEM (문제) - 총 17개 (각 소분류당 1~2개)
-- #####################################################################
-- 소분류 1 (ID 1): Python 변수 선언
...
-- #####################################################################
-- 6. CHOICE (객관식 선택지) - P_ID 1, 3, 4, 6, 7, 8, 9, 10, 11, 13, 14, 16에 연결
-- #####################################################################
INSERT INTO CHOICE (PROBLEM_ID, CONTENT, IS_CORRECT, CORRECT_DESCRIPTION, CREATED_AT, UPDATED_AT)
VALUES (1, 'count = 10', TRUE, 'Python은 별도의 키워드 없이 바로 할당합니다.', NOW(), NOW()),
       ...
       (3, '11', TRUE, '공백을 포함하여 총 11글자입니다.', NOW(), NOW()),
       (3, '10', FALSE, NULL, NOW(), NOW()),
       (3, '12', FALSE, NULL, NOW(), NOW()),
       (6, '1', TRUE, '리스트 인덱스는 0부터 시작합니다.', NOW(), NOW()),
       (6, '2', FALSE, NULL, NOW(), NOW()),
       (6, '3', FALSE, NULL, NOW(), NOW()),
       (7, 'MyClass obj = new MyClass();', TRUE, 'new 키워드를 사용하여 객체를 생성합니다.', NOW(), NOW()),
       (7, 'obj = MyClass.create();', FALSE, NULL, NOW(), NOW()),
       (7, 'var obj = new MyClass();', FALSE, NULL, NOW(), NOW()),
       (8, 'const', TRUE, 'const는 재할당이 불가능합니다.', NOW(), NOW()),
       (8, 'let', FALSE, NULL, NOW(), NOW()),
       (8, 'var', FALSE, NULL, NOW(), NOW()),
       (16, 'document.getElementById()', TRUE, 'getElementById는 ID로 요소를 찾습니다.', NOW(), NOW()),
       (16, 'document.querySelector()', FALSE, NULL, NOW(), NOW()),
       (16, 'document.getElementsByClass()', FALSE, NULL, NOW(), NOW());

-- #####################################################################
-- 7. SOLUTION (주관식 정답) - P_ID 2, 5, 12, 15, 17에 연결
-- #####################################################################
INSERT INTO SOLUTION (PROBLEM_ID, CONTENT, CORRECT_DESCRIPTION, CREATED_AT, UPDATED_AT)
VALUES (2, 'a, b = b, a', '파이썬의 튜플 언패킹을 이용한 간결한 방법입니다.', NOW(), NOW()),
       (5, 'for i in range(1, 6): print(i)', 'range(1, 6)은 1부터 5까지의 정수를 생성합니다.', NOW(), NOW()),
       (12, 'public Car() {}', '클래스 이름과 동일하며, 반환 타입이 없습니다.', NOW(), NOW()),
       (15, 'let x = 10; console.log(x);', 'let 또는 const를 사용하여 변수를 선언합니다.', NOW(), NOW()),
       (17, 'document.getElementById("myButton").addEventListener("click", function() {});',
        '가장 일반적인 이벤트 리스너 추가 방법입니다.', NOW(), NOW());

-- #####################################################################
-- 8. HINTS (힌트) - P_ID 1, 2, 5에 연결
-- #####################################################################
